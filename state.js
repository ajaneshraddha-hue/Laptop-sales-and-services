...
class StateManager {
  ...
  submitOrderPaymentProof(addressId, paymentMethod, screenshotData) {
    ...
    this.state.currentOrder = orderId;
    this.addNotification(`Payment proof submitted for order ${orderId}.`);
    this.saveState();
    return { success: true, orderId };
  }

  verifyPayment(ticketId, approved) {
    const ticket = (this.state.serviceTickets || []).find(t => t.id === ticketId);
    if (!ticket) return { success: false, message: "Payment request not found." };
    ticket.paymentStatus = approved ? "approved" : "rejected";
    ticket.paymentVerifiedAt = new Date().toLocaleString('en-IN');
    const customer = (this.state.registeredUsers || []).find(u => u.email.toLowerCase() === (ticket.customerEmail || '').toLowerCase());
    if (customer) customer.paymentAccessApproved = approved;
    if (this.state.currentUser && this.state.currentUser.email.toLowerCase() === (ticket.customerEmail || '').toLowerCase()) {
      this.state.currentUser.paymentAccessApproved = approved;
    }
    this.addNotification(`Payment for ${ticketId} was ${approved ? 'approved' : 'rejected'} by admin.`);
    this.saveState();
    return { success: true };
  }

  verifyOrderPayment(orderId, approved) {
    const order = (this.state.orders || []).find(o => o.id === orderId);
    if (!order) return { success: false, message: "Order payment not found." };
    order.paymentStatus = approved ? "approved" : "rejected";
    order.paymentVerifiedAt = new Date().toLocaleString('en-IN');
    this.addNotification(`Payment for ${orderId} was ${approved ? 'approved' : 'rejected'} by admin.`);
    this.saveState();
    return { success: true };
  }
